...
from tests.validation.test_truth import PROFILE
...
def run(tailored=None, profile=PROFILE):
    tailored_result = tailored if tailored is not None else {
        **make_good_tailored(),
    }
...
class TestSeverityAggregation:
    ...
    def test_truth_failure_makes_overall_fail(self) -> None:
        tailored = {
            ...
            "resume": {
                ...
                "summary": {"text":
                            "Expert in pytorch kubernetes rust.",
                            "evidence_refs": []},
                "skills": [],
                "experience": [],
                "projects": [],
    ...
    def test_ats_warning_caps_at_warn(self) -> None:
        ...
        # over-long bullet triggers an A7 warning only
        ...
        assert not any(c.status == "failed"
                       for c in outcome.report.truth.checks)
...
class TestMetrics:
    def test_metrics_present_and_explainable(self) -> None:
        ...
        python_entry = next(
            (k for k in metrics.keyword_counts if k.term == "python"), None
        )
...
class TestDeterminism:
    def test_repeat_runs_identical(self) -> None:
        ...
        for section in ("truth", "ats"):
            for check_first in first[section]["checks"]:

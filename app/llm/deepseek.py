"""DeepSeek vendor adapter: OpenAI-compatible wire format with custom base URL."""
...
class DeepSeekClient(OpenAICompatibleClient):
    def __init__(
        ...
        base_url = settings.deepseek_base_url.strip().rstrip("/") or _DEFAULT_BASE_URL
        # Use provider-specific model if configured, otherwise generic setting
        model = settings.deepseek_model.strip() or settings.jarvis_llm_model.strip()

    ...
    *,
    ...
    if not soft.target_roles:
        ...
        title_component = ComponentResult(
            ...
            title_points,
            _TITLE_MAX,
            "matched" if title_points > 0 else "mismatch",
            reason,
        )
    ...
    if soft.required_skills:
        ...
        required_status = (
            "matched"
            if required_points == _REQUIRED_SKILLS_MAX
            else "partial"
            if required_matches
            else "missing"
        )
    ...
    breakdown["skills"] = {
        "required": ComponentResult(
            ...
            (
                ...
                if soft.required_skills
                else "no required skills supplied"
            ),
        ),
        "preferred": ComponentResult(
            ...
            (
                ...
                if soft.preferred_skills
                else "no preferred skills supplied"
            ),
        ),
    }
    ...
    if not hard_locations(prefs):
        location_component = ComponentResult(
            ...
            _LOCATION_MAX,
            "not_requested",
            "no location requirement supplied",
        )
        ...
        requested_keys = {base_normalize(location) for location in prefs.hard.locations}
    ...
    if not prefs.hard.experience_levels and not soft.prefer_internship_fresher:
        ...
        level_component = ComponentResult(
            ...
            _LEVEL_MAX,
            "neutral",
            "level not detectable from verified fields",
        )
        ...
        level_component = ComponentResult(
            ...
            _LEVEL_MAX,
            "matched",
            f"detected level {features.level.value}",
        )
        ...
        level_component = ComponentResult(
            ...
            _LEVEL_MAX,
            "matched",
            "intern/fresher preference satisfied",
        )
    else:
        level_component = ComponentResult(
            "experience",
            ...
            _LEVEL_MAX,
            "mismatch",
            f"detected level {features.level.value} outside preference",
        )
    ...
    if features.created_at is None:
        freshness_component = ComponentResult(
            ...
            _FRESHNESS_MAX,
            "neutral",
            "posting date unavailable from source",
        )
    else:
    ...
    if not prefs.soft.workplace_types and not prefs.hard.employment_types:
        employment_component = ComponentResult(
            ...
            _EMPLOYMENT_MAX,
            "not_requested",
            "no employment type preference supplied",
        )
        ...
        employment_component = ComponentResult(
            "employment_type",
            float(_EMPLOYMENT_MAX),
            _EMPLOYMENT_MAX,
            "matched",
            f"type {features.employment.value} matches preference",
        )
    else:
        employment_component = ComponentResult(
            "employment_type",
            ...
            _EMPLOYMENT_MAX,
            "mismatch",
            f"type {features.employment.value} outside preference",
        )
    ...
    if soft.salary_min is None:
        ...
        salary_component = ComponentResult(
            "salary",
            float(_SALARY_MAX) if meets else 0.0,
            _SALARY_MAX,
    ...
    total = sum(component.points for component in _iter_components(breakdown))

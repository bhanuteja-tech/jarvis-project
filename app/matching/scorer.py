...
def score_pair(candidate: CandidateView, job_view: JobView) -> dict[str, Any]:
    ...
    fallback_count = sum(1 for c in components if "unavailable" in c.reason or "fallback" in c.reason)
...
def _skills_required(
    candidate: CandidateView, job_view: JobView
) -> ComponentResult:
    if not job_view.required_skills:
        return ComponentResult("skills_required", 30, 30, "not_requested",
                               "JD analysis states no required skills")
    matched = job_view.required_skills & candidate.skill_names
    points = round(30 * len(matched) / len(job_view.required_skills), 2)
    status = "matched" if len(matched) == len(job_view.required_skills) else (
...
def _skills_preferred(
    candidate: CandidateView, job_view: JobView
) -> ComponentResult:
    if not job_view.preferred_skills:
        return ComponentResult("skills_preferred", 10, 10, "not_requested",
                               "JD analysis states no preferred skills")
    matched = job_view.preferred_skills & candidate.skill_names
    points = round(10 * len(matched) / len(job_view.preferred_skills), 2)
    status = "matched" if len(matched) == len(job_view.preferred_skills) else (
        "partial" if matched else "mismatch"
    )
    return ComponentResult(
        "skills_preferred",
...
def _experience(candidate: CandidateView, job_view: JobView) -> ComponentResult:
    ...
    if required_min is None and required_max is None:
        ...
        if floor is None:
            return ComponentResult(
                "experience", 10, 20, "not_requested",
    ...
    if required_min is not None and years < required_min:
        ...
        return ComponentResult(
            "experience", float(points), 20, "partial",
            "Candidate has insufficient experience "
            f"({years:g} of at least {required_min:g} years)",
        )
    if required_max is not None and years > required_max:
        return ComponentResult(
            "experience", 20, 20, "matched",
...
def _location(candidate: CandidateView, job_view: JobView) -> ComponentResult:
    if job_view.is_remote_job:
        if candidate.remote_pref is True:
            return ComponentResult("location", 12, 12, "matched",
                                   "Candidate accepts remote work")
        if candidate.relocation is True:
            return ComponentResult("location", 12, 12, "matched",
                                   "Remote role; relocation accepted anyway")
        if candidate.remote_pref is False:
            return ComponentResult("location", 0, 12, "mismatch",
                                   "Remote role but candidate prefers onsite")
        return ComponentResult("location", 9, 12, "neutral",
                               "Remote role; candidate preference unknown")

    if job_view.location_key is None:
        return ComponentResult("location", 6, 12, "neutral",
    ...
    if candidate.relocation is True:
        return ComponentResult("location", 9, 12, "partial",
...
def _employment(candidate: CandidateView, job_view: JobView) -> ComponentResult:
    if not candidate.employment_types:
        return ComponentResult("employment_type", 10, 10, "not_requested",
                               "Candidate stated no employment-type preference")
    if job_view.employment is None:
        return ComponentResult("employment_type", 5, 10, "neutral",
                               "Job employment type unknown")
    if job_view.employment in candidate.employment_types:
        return ComponentResult("employment_type", 10, 10, "matched",
                               f"Employment type '{job_view.employment}' matches preference")
    return ComponentResult("employment_type", 0, 10, "mismatch",
...
def _education(candidate: CandidateView, job_view: JobView) -> ComponentResult:
    ...
    if required_rank is None:
        return ComponentResult("education", 8, 8, "not_requested",
                               "JD states no education requirement")
    candidate_rank = candidate.highest_edu_rank
    if candidate_rank is None:
        return ComponentResult("education", 4, 8, "neutral",
                               "candidate education unknown")
    if candidate_rank >= required_rank:
        return ComponentResult("education", 8, 8, "matched",
                               "Candidate education meets requirement")
    if required_rank - candidate_rank == 1:
        return ComponentResult("education", 3.2, 8, "partial",
                               "Candidate education one level below requirement")
    return ComponentResult("education", 0, 8, "mismatch",
                           "Candidate education below requirement")
...
def _level(candidate: CandidateView, job_view: JobView) -> ComponentResult:
    ...
    if candidate_rank is None or jd_level_rank is None:
        return ComponentResult("level", 2.5, 5, "neutral",
                               "seniority not detectable on one side")
    distance = abs(candidate_rank - jd_level_rank)
    if distance == 0:
        return ComponentResult("level", 5, 5, "matched", "Seniority aligns with JD")
    if distance == 1:
        return ComponentResult("level", 3, 5, "partial",
                               "Seniority adjacent to JD expectation")
    return ComponentResult("level", 0, 5, "mismatch",
                           "Seniority gap versus JD expectation")
...
def _salary(candidate: CandidateView, job_view: JobView) -> ComponentResult:
    ...
    if expected is None:
        return ComponentResult("salary", 5, 5, "not_requested",
                               "Candidate stated no minimum salary")
    offered_low = job_view.offered_min
    offered_high = job_view.offered_max
    if offered_low is None and offered_high is None:
        return ComponentResult("salary", 2.5, 5, "neutral",
                               "salary unavailable")
    if (
        job_view.offered_currency is not None
        and candidate.salary_currency is not None
        and job_view.offered_currency != candidate.salary_currency
    ):
        return ComponentResult("salary", 0, 5, "mismatch",
                               "currency mismatch between offer and expectation")
    high = offered_high if offered_high is not None else offered_low
    low = offered_low if offered_low is not None else offered_high
    if high is not None and high >= expected:
        return ComponentResult("salary", 5, 5, "matched",
                               "Offered salary meets expected minimum")
    if low is not None and high is not None and low <= expected <= high:
        return ComponentResult("salary", 3, 5, "partial",
                               "Expected salary falls inside offered range")
    return ComponentResult("salary", 0, 5, "mismatch",
                           "Salary range is below expected minimum")
